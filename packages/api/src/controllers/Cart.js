...
class CartController {
  ...
  static async addToCart(request, h) {
    try {
      ...
      if (!product) {
        return h.response({ statusCode: 404, message: 'Product not found' }).code(404);
      }
      ...
      const vendor = await Vendor.findOne({ userId: product.user._id });
      if (!vendor) {
        return h.response({ statusCode: 404, message: 'Vendor not found' }).code(404);
      }
      ...
      if (existingCart) {
        ...
        if (existingItem) {
          ...
          await cartServices.updateItemQuantity(identifier, existingItem._id, newQuantity);
  ...
  static async updateQuantity(request, h) {
    try {
      ...
      if (!cart) {
        return h.response({ statusCode: 404, message: 'Cart not found' }).code(404);
      }
      ...
      if (!cartItem) {
        return h.response({ statusCode: 404, message: 'Cart item not found' }).code(404);
      }
